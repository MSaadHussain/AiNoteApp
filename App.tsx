...
import { performSemanticSearch, transcribeAudio, organizeNote, convertImageToNote } from './services/geminiService';
import { extractTextFromPdfBlob } from './services/pdfService';
...
const App: React.FC = () => {
    ...
    const handleProcessPdf = async (blob: Blob, subject?: string) => {
        ...
        setBgTask({ status: 'processing', message: 'Extracting PDF Text...', details: 'Reading document pages...' });

        try {
            const { text, pageCount } = await extractTextFromPdfBlob(blob, (msg) => {
                setBgTask({ status: 'processing', message: 'Reading PDF...', details: msg });
            });

            if (!text.trim()) {
                setBgTask({ status: 'error', message: 'No Text Found', details: 'This PDF may be scanned/image-based. Try uploading as an image instead.' });
                setTimeout(() => setBgTask(null), 5000);
                return;
            }

            const finalSubject = subject || 'General';
            const fileName = `PDF Document (${pageCount} pages)`;
            ...
            const newNote: Note = {
                ...
                type: 'pdf',
                rawContent: text,
                summary: `Extracted text from ${pageCount} page PDF document. Select text in Smart Reader to ask AI questions.`,
                sections: [],
                tags: ['pdf', 'extracted'],
            ...
            handleNoteCreated(newNote);
            setBgTask({ status: 'success', message: 'PDF Ready!', details: `Extracted text from ${pageCount} pages. Open to read & ask AI.` });
            setTimeout(() => setBgTask(null), 4000);

        } catch (error) {
            console.error(error);
            setBgTask({ status: 'error', message: 'Extraction Failed', details: 'Could not read this PDF.' });
            setTimeout(() => setBgTask(null), 5000);
        }
    };
        ...
        <div className="flex flex-col md:flex-row h-screen bg-stone-100 font-sans text-ink overflow-hidden">
            ...
            <div className="md:hidden h-16 bg-white border-b border-stone-200 flex items-center justify-between px-4 z-40 flex-shrink-0">
                ...
                <div className="flex items-center gap-2">
                    ...
                    <h1 className="font-hand font-bold text-xl text-stone-800">NoteNest</h1>
                </div>
